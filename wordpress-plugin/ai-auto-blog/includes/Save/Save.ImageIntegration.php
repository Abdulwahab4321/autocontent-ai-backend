...
?>
        ...
        <template id="aab-image-settings-template">
            <div class="aab-box aab-featured-box">
                <h2>Featured Image Settings</h2>

                <div class="row">
                    <label><input type="checkbox" name="aab_feat_generate" id="aab_feat_generate" <?php echo $feat_generate ? 'checked' : ''; ?>> Generate Featured Image</label>
                </div>

                <div class="row">
                    <label><strong>Image Extraction Method</strong></label><br>
                    <select name="aab_feat_image_method" id="aab_feat_image_method">
                        <option value="dalle" <?php selected($feat_method, 'dalle'); ?>>Dall-e (default)</option>
                    </select>
                </div>

                <div class="row">
                    <label><strong>Select Image Model</strong></label><br>
                    <select name="aab_feat_image_model" id="aab_feat_image_model">
                        <option value="dall-e-3" <?php selected($feat_model, 'dall-e-3'); ?>>DALL·E 3</option>
                    ...
                    <p id="aab_feat_model_note" class="muted" style="display:<?php echo (stripos($feat_model, 'gpt-image') === 0) ? 'block' : 'none'; ?>;">
                        Your organization must be verified to use the model <code><?php echo esc_html($feat_model); ?></code>.
                        Please go to: <a href="https://platform.openai.com/settings/organization/general" target="_blank" rel="noopener">OpenAI Organization Settings</a> and verify. Propagation can take up to 15 minutes.
                    </p>
                </div>

                <div class="row">
                    <label><strong>Select Image Size</strong></label><br>
                    <select name="aab_feat_image_size" id="aab_feat_image_size">
                        <option value="1024x1024" <?php selected($feat_size, '1024x1024'); ?>>1024x1024</option>
                        <option value="512x512" <?php selected($feat_size, '512x512'); ?>>512x512</option>
                        <option value="256x256" <?php selected($feat_size, '256x256'); ?>>256x256</option>
                    </select>
                </div>

                <!-- NEW: Featured "Get Image By Prompt" -->
                <div class="row" style="margin-top: 10px;">
                    <label><input type="checkbox" name="aab_feat_get_image_by_prompt" id="aab_feat_get_image_by_prompt" <?php echo $feat_get_by_prompt ? 'checked' : ''; ?>> Get Featured Image By Prompt</label>
                </div>

                <div class="row" id="aab_feat_custom_prompt_row" style="<?php echo $feat_get_by_prompt ? '' : 'display:none;'; ?>">
                    <label>Featured Custom Prompt (used when 'Get Featured Image By Prompt' is checked)</label><br>
                    <textarea name="aab_feat_custom_prompt" id="aab_feat_custom_prompt" rows="3" class="large-text"><?php echo $feat_custom_prompt; ?></textarea>
                </div>
            </div>

            <div class="aab-box aab-content-box">
                <h2>Content Image Settings</h2>

                <div class="row" style="margin: 10px 0;">
                    <label><strong>Multiple Image Extraction Method</strong></label><br>
                    <select name="aab_content_image_method" id="aab_content_image_method">
                        <option value="dalle" <?php selected($content_method, 'dalle'); ?>>Dall-e (default)</option>
                    </select>
                </div>
                ...
                <div class="row" style="margin: 10px 0;">
                    ...
                    <select name="aab_content_image_model" id="aab_content_image_model">
                        <option value="dall-e-3" <?php selected($content_model, 'dall-e-3'); ?>>DALL·E 3</option>
                    ...
                    <p id="aab_content_model_note" class="muted" style="display:<?php echo (stripos($content_model, 'gpt-image') === 0) ? 'block' : 'none'; ?>;">
                        Your organization must be verified to use the model <code><?php echo esc_html($content_model); ?></code>.
                        Please go to: <a href="https://platform.openai.com/settings/organization/general" target="_blank" rel="noopener">OpenAI Organization Settings</a> and verify. Propagation can take up to 15 minutes.
                    </p>
                </div>
                ...
                <div class="row" style="margin: 10px 0;">
                    ...
                    <select name="aab_content_image_size" id="aab_content_image_size">
                        <option value="1024x1024" <?php selected($content_size, '1024x1024'); ?>>1024x1024</option>
                        <option value="512x512" <?php selected($content_size, '512x512'); ?>>512x512</option>
                        <option value="256x256" <?php selected($content_size, '256x256'); ?>>256x256</option>
                    </select>
                </div>
                
                <div class="row" style="margin-top:10px;">
                    <label><input type="checkbox" name="aab_get_image_by_title" id="aab_get_image_by_title" <?php echo $get_by_title ? 'checked' : ''; ?>> Get Image By Title</label><br>
                    <label ><input type="checkbox" name="aab_get_image_by_prompt" id="aab_get_image_by_prompt" <?php echo $get_by_prompt ? 'checked' : ''; ?>> Get Image By Prompt</label>
                </div>
                
                <div class="row" id="aab_custom_prompt_row" style="<?php echo $get_by_prompt ? '' : 'display:none;'; ?>">
                    <label>Custom Prompt (used when 'Get Image By Prompt' is checked)</label><br>
                    <textarea name="aab_content_custom_prompt" id="aab_content_custom_prompt" rows="3" class="large-text"><?php echo $content_custom_prompt; ?></textarea>
                </div>

                <div class="row" style="margin-top:10px;">
                    <label><input type="checkbox" name="aab_set_keyword_as_alt" id="aab_set_keyword_as_alt" <?php echo $set_keyword_alt ? 'checked' : ''; ?>> Set Image Keyword as Alt Text</label><br>
                    <label ><input type="checkbox" name="aab_search_by_parent_keyword" id="aab_search_by_parent_keyword" <?php echo $search_by_parent_keyword ? 'checked' : ''; ?>> Search Images by Parent Keyword</label>
                </div>

                <div class="row" style="margin: 10px 0;">
                    <label><strong>Number of Images to Import</strong></label><br>
                    <select name="aab_content_num_images" id="aab_content_num_images">
                        <option value="1" <?php selected($content_num, 1); ?>>1</option>
                ...
                <div class="row">
                    <label><strong>Image Distribution Method</strong></label><br>
                    <select name="aab_content_dist" id="aab_content_dist">
                        <option value="fixed" <?php selected($content_dist, 'fixed'); ?>>Fixed</option>
                        <option value="dynamic" <?php selected($content_dist, 'dynamic'); ?>>Dynamic</option>
                    </select>
                </div>

                <div class="row" style="margin: 10px 0;">
                    <label><strong>Position for Image #1</strong></label><br>
                    <select name="aab_content_pos_1">
                        <option value="top" <?php selected($content_pos_1, 'top'); ?>>Top of the Content</option>
                        <option value="middle" <?php selected($content_pos_1, 'middle'); ?>>Middle of the Content</option>
                        <option value="bottom" <?php selected($content_pos_1, 'bottom'); ?>>Bottom of the Content</option>
                    </select>
                </div>

                <div class="row">
                    <label><strong>Position for Image #2</strong></label><br>
                    <select name="aab_content_pos_2">
                        <option value="top" <?php selected($content_pos_2, 'top'); ?>>Top of the Content</option>
                        <option value="middle" <?php selected($content_pos_2, 'middle'); ?>>Middle of the Content</option>
                        <option value="bottom" <?php selected($content_pos_2, 'bottom'); ?>>Bottom of the Content</option>
                    </select>
                </div>

                <div class="row" style="margin: 10px 0;">
                    <label><strong>Position for Image #3</strong></label><br>
                    <select name="aab_content_pos_3">
                        <option value="top" <?php selected($content_pos_3, 'top'); ?>>Top of the Content</option>
                        <option value="middle" <?php selected($content_pos_3, 'middle'); ?>>Middle of the Content</option>
                        <option value="bottom" <?php selected($content_pos_3, 'bottom'); ?>>Bottom of the Content</option>
                    </select>
                </div>

                <div class="row">
                    <label><strong>Content Image Size (WP)</strong></label><br>
                    <select name="aab_content_wp_image_size" id="aab_content_wp_image_size">
                        <?php foreach ($wp_sizes as $s): ?>
                            <option value="<?php echo esc_attr($s); ?>" <?php selected($content_wp_image_size, $s); ?>><?php echo esc_html($s); ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
            </div>
    ...
    private static function process_images_for_post($post_id, $post, $campaign_id)
    {
        ...
        if (is_wp_error($res)) {
            ...
            try {
                ...
                // error_log('AAB ImageIntegration: apply_campaign_alt_rules() executed for post ' . $post_id);
            } catch (\Throwable $e) {
                // error_log('AAB ImageIntegration: apply_campaign_alt_rules() threw: ' . $e->getMessage());
            }
        }
    }
